...
_periodicityMonths = {"monthly": 1, "quaterly": 3, "sixmonthly": 6, "yearly": 12}
...
class CrossoveredBudget(models.Model):
    ...
    def action_create_period(self):
        ...
        for budget in self.filtered(
            lambda b: not b.crossovered_budget_line_ids and b.state == "draft"
        ):
            budget_posts = budget.budget_tmpl_id.budget_post_ids
            periodicity_months = False
            if budget.budget_tmpl_id.periodicity:
                periodicity_months = _periodicityMonths[
                    budget.budget_tmpl_id.periodicity
                ]
            vals = {"crossovered_budget_id": budget.id, "planned_amount": 0.0}
            if not periodicity_months:
                ...
                ds = fields.Date.to_date(budget.date_from)
                date_to = fields.Date.to_date(budget.date_to)
                while ds <= date_to:
