...
class AccountConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        budget_templ_id = literal_eval(
            get_param("account_budget_template.budget_template_id", default="False")
        )
        if (
            budget_templ_id
            and not self.env["crossovered.budget.template"]
            .sudo()
            .browse(budget_templ_id)
            .exists()
        ):
            budget_templ_id = False
        res.update(budget_templ_id=budget_templ_id)
        return res
    ...
    def set_values(self):
        ...
        # a required string
        set_param(
            "account_budget_template.budget_template_id", repr(self.budget_templ_id.id)
        )
