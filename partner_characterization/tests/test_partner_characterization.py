...
class TestPartnerCharacterization(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_name_get(self):
        ...
        self.assertEqual(
            self.activity_type1.with_context(show_activity_name=True).name_get()[0],
            (
                self.activity_type1.id,
                "{} / {}".format(
                    self.activity_type1.activity_id.name, self.activity_type1.name
                ),
            ),
        )
